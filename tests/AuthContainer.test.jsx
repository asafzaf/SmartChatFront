...
jest.mock("../src/components/auth/SignInForm", () => (props) => (
    <div>
        ...
        <button data-testid="flip-signin" onClick={props.handleFlip}>Flip</button>
...
jest.mock("../src/components/auth/SignUpForm", () => (props) => (
    <div>
        ...
        <button data-testid="flip-signup" onClick={props.handleFlip}>Flip</button>
...
describe("AuthContainer", () => {
    ...
    test("shows signup error if signup fails", async () => {
        ...
        await waitFor(() => {
            expect(screen.getByTestId("signup-error")).toHaveTextContent("Email already exists");
        });
    });
    ...
    test("clears error when flipping between forms", async () => {
        ...
        fireEvent.click(screen.getByText("Submit Login"));
        await waitFor(() => {
            expect(screen.getByTestId("login-error")).toHaveTextContent("Wrong password");
        });
        fireEvent.click(screen.getByTestId("flip-signin"));
        await waitFor(() => {
