...
describe("setupChatHandlers", () => {
    ...
    test("should register chat_list handler", () => {
        ...
        const callback = socket.on.mock.calls.find(call => call[0] === "chat_list")[1];
    ...
    test("should handle chat_history with one message (adds thinking message)", () => {
        ...
        const callback = socket.on.mock.calls.find(call => call[0] === "chat_history")[1];
        const messages = [{ chatId: "c1", sender: "user123", text: "hello" }];
        callback(messages);
        expect(setMessages).toHaveBeenCalledWith(expect.arrayContaining([
    ...
    test("should handle chat_created and update state", () => {
        ...
        const callback = socket.on.mock.calls.find(call => call[0] === "chat_created")[1];
        const newChat = {
            _id: "chat42",
